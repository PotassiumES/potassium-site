...
class XRWebGLLayer$1 extends XRLayer {
...
class ClickFilter extends Filter {
...
class ReverseActiveFilter extends Filter {
  constructor() {
    super();
  }
  filter(inputPath, inputValue, filterPath, filterParameters) {
...
class MouseInputSource extends InputSource {
...
class ActionManager {
  ...
  poll() {
    ...
    for (const [actionPath, actionParameters, inputSource] of this._activations.values()) {
      this._deactivations.delete(actionPath);
...
class TouchInputSource extends InputSource {
  constructor(targetElement = document) {
    super();
...
class ActivePickFilter extends Filter {
	constructor(queryInputPath) {
		super();
		this._queryInputPath = queryInputPath;
	}
...
class PickingInputSource extends InputSource {
	constructor() {
		super();
		this._mouse = null;
		this._touch = null;
...
class VirtualKeyboardInputSource extends InputSource {
	constructor() {
		super();
...
const App = class extends EventHandler {
	...
	_makeHand(color) {
		return graph.obj(
			...
			(group, obj) => {
				const body = group.getObjectByName("Body_Cylinder");
				if (!body) {
